...
CREATE TABLE `rentas` (
  ...
  PRIMARY KEY (`idRenta`),
  KEY `ticket_id_ventas` (`ticket_id`),
  KEY `juego_id_ventas` (`juego_id`),
  CONSTRAINT `juego_id_ventas` FOREIGN KEY (`juego_id`) REFERENCES `catjuegos` (`idJuego`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ticket_id_ventas` FOREIGN KEY (`ticket_id`) REFERENCES `tickets` (`idTicket`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `tickets` (
  ...
  KEY `usuario_id_tickets` (`usuario_id`),
  CONSTRAINT `usuario_id` FOREIGN KEY (`usuario_id`) REFERENCES `usuarios` (`idUsuario`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `usuarios` (
  ...
  CONSTRAINT `perfil_id` FOREIGN KEY (`perfil_id`) REFERENCES `catperfiles` (`idPerfil`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `ventas` (
  ...
  `ticket_id` int(11) NOT NULL COMMENT 'Llave foranea que indica el ticket al que está relacionado el presente registro.',
  PRIMARY KEY (`idVenta`),
  KEY `producto_id_ventas` (`producto_id`),
  KEY `ticket_id_ventas` (`ticket_id`),
  CONSTRAINT `producto_id` FOREIGN KEY (`producto_id`) REFERENCES `catproductos` (`idProducto`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ticket_id` FOREIGN KEY (`ticket_id`) REFERENCES `tickets` (`idTicket`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2017-10-16  9:53:20
